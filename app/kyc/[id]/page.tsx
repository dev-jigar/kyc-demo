...
import { 
  User, Mail, Phone, MapPin, Wallet, Calendar, Shield, 
  CheckCircle, Clock, Award, ArrowLeft, Download, 
  CreditCard, Globe, Home, Building2, Flag, Hash, AlertCircle 
} from "lucide-react";
...
export default function CustomerDetailPage() {
  ...
  const downloadReport = async () => {
    window.open(`/api/kyc/report?userId=${id}`, "_blank");
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-medium text-lg">Customer not found</p>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
            ...
            <div className="relative flex flex-col md:flex-row gap-6 items-start md:items-center">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {customer.isActive && (
                    <StatusBadge icon={<CheckCircle className="w-3.5 h-3.5" />} label="KYC Active" variant="success" />
                  )}
                  {user.phoneVerified && (
                    <StatusBadge icon={<Phone className="w-3.5 h-3.5" />} label="Phone Verified" variant="verified" />
                  )}
                  {user.emailVerified && (
                    <StatusBadge icon={<Mail className="w-3.5 h-3.5" />} label="Email Verified" variant="verified" />
                  )}
                  {user.mainProfileCompleted && (
                    <StatusBadge icon={<User className="w-3.5 h-3.5" />} label="Profile Complete" variant="verified" />
                  )}
                  {address?.isDmvVerified && (
                    <StatusBadge icon={<Shield className="w-3.5 h-3.5" />} label="DMV Verified" variant="verified" />
                  )}
                </div>
              </div>
              ...
              {beingId && (
                <div className="bg-white/20 backdrop-blur-md rounded-2xl px-6 py-4 border border-white/30 shadow-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white/80 text-xs font-medium uppercase tracking-wider">User Level</p>
                      <p className="text-3xl font-bold text-white">{beingId.level}</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="bg-gradient-to-r from-slate-50 to-slate-100 px-8 md:px-10 py-6 border-t border-slate-200">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <MetaItem 
                ...
                label="Wallet ID" 
                value={user.walletAddress ? user.walletAddress.slice(0, 10) + "..." : "—"}
                copyable={user.walletAddress}
              />
              <MetaItem 
                icon={<Calendar className="w-5 h-5" />}
                label="Started" 
                value={new Date(customer.startedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} 
              />
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Address Information */}
            {address && (
              <InfoCard title="Address Details" icon={<MapPin className="w-5 h-5" />}>
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    {address.isDefault && <SmallBadge label="Default Address" variant="blue" />}
                    {address.isDmvVerified && <SmallBadge label="DMV Verified" variant="green" />}
                  </div>
                </div>
              </InfoCard>
            )}
            ...
            <InfoCard title="KYC Add-Ons" icon={<Shield className="w-5 h-5" />}>
              <div className="space-y-3">
                {customer.addOns && customer.addOns.length > 0 ? (
                  ...
                  <p className="text-slate-500 text-sm text-center py-4">No add-ons configured</p>
                )}
              </div>
            </InfoCard>

            {/* Account Details */}
            <InfoCard title="Account Details" icon={<User className="w-5 h-5" />}>
              <div className="space-y-3">
                <DetailRow label="Account Status" value={customer.blocked ? "Blocked" : "Active"} 
                  valueClass={customer.blocked ? "text-red-600" : "text-emerald-600"} />
                ...
                <DetailRow label="Privacy Accepted" value={user.privacyAccepted ? "Yes" : "No"} />
                <DetailRow label="Public Profile" value={user.isPublic ? "Yes" : "No"} />
              </div>
            </InfoCard>
...
function StatusBadge({ icon, label, variant }: any) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold backdrop-blur-sm border ${variants[variant as keyof typeof variants]}`}>
      {icon}
      {label}
    </span>
  );
...
function SmallBadge({ label, variant }: any) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${variants[variant as keyof typeof variants]}`}>
      {label}
    </span>
  );
}
...
function AddonChip({ label, status }: any) {
  ...
  return (
    <div className={`flex items-center justify-between p-4 rounded-xl border ${config.border} ${config.bg}`}>
      <div className="flex items-center gap-3">
        <div className={`w-2.5 h-2.5 rounded-full ${config.dot} animate-pulse`}></div>
        <span className={`font-semibold ${config.text}`}>{label}</span>
      </div>
      <span className={`text-xs font-bold ${config.text} uppercase tracking-wider`}>{status}</span>
    </div>
  );
}
