...
import {
  ...
  IReverificationRequestOccurrenceResponse,
  IReverificationRequestResponse,
} from "@/src/types";
import { cancelReverificationType } from "../../const";
import { CancelReverificationModal } from "./CancelReverification";

export function ReverificationDetailPage({ id }: { id: string }) {
  const router = useRouter();

  const [data, setData] = useState<IReverificationRequestResponse | null>(null);
  ...
  async function loadData() {
    try {
      ...
      const response: IReverificationRequestResponse = await res.json();
      setData(response);
    } catch (error) {
  ...
  async function cancelReverification(
    ...
    try {
      const response = await fetch(
        `/api/kyc/reverifications/${taskId}/cancel`,
        {
          method: "PUT",
          headers: {
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {data.status === EReverificationStatus.Pending && (
              <button
          ...
          <div className="relative bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 p-8 md:p-10">
            ...
            <div className="relative flex flex-col md:flex-row gap-6 items-start md:items-center">
              ...
              <div className="relative">
                ...
                {data.status === EReverificationStatus.Completed && (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                        data.status === EReverificationStatus.Completed
                          ...
                          : data.status === EReverificationStatus.Pending
                            ...
                            : data.status === EReverificationStatus.Rejected
                ...
                {data.frequency === EReverificationFrequency.Recurring && (
                  <>
                    <InfoItem
                      label="Repeat Every"
              ...
              data.reverificationRequestOccurrences.length > 0 ? (
                <div className="space-y-4">
                  {data.reverificationRequestOccurrences.map(
                    (
                      occurrence: IReverificationRequestOccurrenceResponse,
                      idx: number,
                    ) => (
                      <OccurrenceCard
                        key={occurrence.id}
...
function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold backdrop-blur-sm border ${variants[variant]}`}
    >
      {icon}
      {label}
    </span>
...
function OccurrenceCard({
  ...
  const config = statusConfig[occurrence.status] || statusConfig.PENDING;

  const StatusIcon = config.icon;

  return (
    <div
      className={`border-2 ${config.border} ${config.bg} rounded-xl p-5 hover:shadow-md transition-shadow`}
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div
            className={`${config.bg} border ${config.border} rounded-full p-1.5`}
          >
