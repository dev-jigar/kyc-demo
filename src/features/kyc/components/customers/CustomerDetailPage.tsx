...
import { EAddOnResultStatus, ILedgerDataResponse } from "@/src/types";
import { VdtCards } from "../individualVDTCards";
import { AttachmentsCard } from "./Attachments";
import { ReverificationListPage } from "../reverification";

export function CustomerDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();

  const [customer, setCustomer] = useState<ILedgerDataResponse>(null);
          ...
          >
            <div className="grid grid-cols-2 gap-6">
              ...
              <InfoField
                ...
                label="BEING LEVEL"
                value={(user?.beingId?.level as unknown as string) || "—"}
              />
            </div>
          </SectionCard>
...
function StatusBadge({ icon, label }: { icon: JSX.Element; label: string }) {
  return (
